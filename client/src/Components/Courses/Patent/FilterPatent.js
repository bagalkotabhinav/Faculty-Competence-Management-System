...
const FilterPatent = () => {
  ...
  useEffect(() => {
    ...
    context.data.getPatents()
      .then((response) => {
        // Filter papers by userId and date range
        const filteredPatents = response.filter(patent => 
            patent.User.id === authUser.id && 
            patent.date >= startDate && 
            patent.date <= endDate
        );
    ...
    isLoading ?
      ...
      : <div className="wrap main--grid">
          ...
          {patents.length > 0 ? (
            ...
            <p>No patents found for the selected date range.</p>
          )}
      </div>
  );
}

export default FilterPatent;